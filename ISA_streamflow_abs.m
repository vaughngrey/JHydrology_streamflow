...
%opts_rho_char = int2str(opts.rho * 10);
%opts_k_char = int2str(opts.k);
%rootdir = append(inputdir,'FINAL_MOD_abs_007_LSTM/');
%datafile_predict = [rootdir 'metadata_predict.csv'];

% Absolute 0.39 / 0.61 SSN
%inputdir = './input_MM_PV/';
%opts.rho = 0.2;
%opts.k = 4;
%opts.perf.AbsPerf = true;           % True if an absolute performance measure, False if a relative performance measure
%opts.perf.epsilon = 0.61;           % Threshold of good performance
%opts_rho_char = int2str(opts.rho * 10);
%opts_k_char = int2str(opts.k);
%rootdir = append(inputdir,'FINAL_MOD_abs_039_SSN/');
%datafile_predict = [rootdir 'metadata_predict.csv'];

% Absolute 0.39 / 0.61 LSTM
inputdir = './input_MM_PV/';
opts.rho = 0.1;
opts.k = 12;
opts.perf.AbsPerf = true;           % True if an absolute performance measure, False if a relative performance measure
...
opts_rho_char = int2str(opts.rho * 10);
opts_k_char = int2str(opts.k);
rootdir = append(inputdir,'FINAL_MOD_abs_039_LSTM/');
datafile_predict = [rootdir 'metadata_predict.csv'];
